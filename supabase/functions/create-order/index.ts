...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!event_id || !participant?.name || !participant?.email || !participant?.phone || !participant?.shipping_address || !participant?.city || !participant?.pincode || !sportDistances[participant.sport_category]?.includes(participant.distance_category)) return json({ error: 'Please complete every registration detail and choose a valid distance for your selected sport.' }, 400)
